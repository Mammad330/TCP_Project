    ...
    for wifi in wifi_list:
        try:
            if "server" in wifi:
                ...
                print("Connected to wifi: "+ str(wifi))
                list_network_ips = scanning_ip_exceptme(get_my_ip())
                print("Active IP addresses in local network")
                ...
                for ip in list_network_ips:
                    try:
                        ...
                        try:
                            ...
                            if (obj_recv.client_detect_action(recv_sign, recv_message)) == True:
                                print("Preparing for broadcasting")
                                # path needed---in case, you are final reciever
                                s.close()
                                print(
                                    "Server is created will be created with SSID " + server_ssid + "password" + server_passcode)
